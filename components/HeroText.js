       ...
       transition={{ duration: 0.6, ease: "easeOut" }}
        className='mb-6 text-lg sm:text-xl md:text-2xl lg:text-3xl text-orange-400 font-semibold' > Hello, My Name Is <span className='font-bold text-sky-500 ml-2'>{name}</span>
          ...
          variants={{ hidden: { opacity: 0, x: -40 }, visible: { opacity: 1, x: 0 } }}
           transition={{ duration: 0.6, ease: "easeOut" }} className='font-black text-3xl md:text-5xl lg:text-7xl tracking-wide text-mainTxt dark:text-mainTxt-dark mt-6 sm:mt-8 font-spartan' > {title1} </motion.h2>
            <motion.h2
             variants={{ hidden: { opacity: 0, x: -40 }, visible: { opacity: 1, x: 0 } }}
              transition={{ duration: 0.6, ease: "easeOut" }}
               className='block text-3xl md:text-5xl lg:text-7xl mt-5 font-black tracking-wide text-mainTxt dark:text-mainTxt-dark font-spartan' > {title2} </motion.h2>
                <motion.div
                 variants={{ hidden: { opacity: 0, x: -40 }, visible: { opacity: 1, x: 0 } }} 
